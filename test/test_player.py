...
def full_episode_set() -> EpisodeSet:
	return EpisodeSet(video_file="video_file", audio_file="audio_file", subtitles_file="subtitles_file")
...
def test_constructor_player_on_full_episode(dummy_constructor_player, full_episode_set):
	...
	assert constructed_command == ["kek_exec", "video_file", "/dub", "audio_file", "/sub", "subtitles_file"]

def test_constructor_player_on_not_full_episode(dummy_constructor_player, episode_set_without_audio, episode_set_without_subtitles):
	constructed_command = dummy_constructor_player._construct_command(episode_set_without_audio)
	assert constructed_command == ["kek_exec", "video_file", "/sub", "subtitles_file"]

	constructed_command = dummy_constructor_player._construct_command(episode_set_without_subtitles)
